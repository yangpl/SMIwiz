...
void do_lsrtm(sim_t *sim, acq_t *acq)
{
  ...
  */
  if(!getparint("niter", &fwi->niter)) fwi->niter = 10;/* number of iterations */
  if(!getparint("restart", &fwi->restart)) fwi->restart = fwi->niter;/* restart iterations */
  ...
  while(fwi->iter<fwi->niter){
    ...
    for(ipar=0; ipar<fwi->npar; ipar++){
      if(fwi->idxpar[ipar]==1) memcpy(&g1[0][0][0], &cg_x[ipar][0][0][0], sim->n123*sizeof(float));
      if(fwi->idxpar[ipar]==2) memcpy(&g2[0][0][0], &cg_x[ipar][0][0][0], sim->n123*sizeof(float));
    }//end for ipar
    ...
    for(irec=0; irec<acq->nrec; irec++){
      for(it=0; it<sim->nt; it++){
	sim->dres[irec][it] = sim->dobs[irec][it] - sim->dcal[irec][it];
	sim->dres[irec][it] *= acq->wdat[irec][it];//muting direct wave should happen here
    ...
    //start CG iterations
    for(iter_cg=0; iter_cg<fwi->restart; iter_cg++){
      if(iproc==0) {
	...
	for(i3=0; i3<sim->n3; i3++){
      ...
      fwi->iter++;//increase counter for outer loop
    }//end for iter_cg
    //==============================================================
  }//end while
