...
{ 
  ...
  if(sim->aniso>0){
    if(flag==0){
      sim->ph0 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
      sim->pv0 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
    }else if(flag==1){
      sim->ph1 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
      sim->pv1 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
    }else if(flag==2){
      sim->ph2 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
      sim->pv2 = alloc3float(sim->n1pad, sim->n2pad, sim->n3pad);
    }
  }
...
void fdtd_null(sim_t *sim, int flag)
{
  ...
  if(sim->aniso>0){
    if(flag==0){
      memset(&sim->ph0[0][0][0], 0, sim->n123pad*sizeof(float));
      memset(&sim->pv0[0][0][0], 0, sim->n123pad*sizeof(float));
    }else if(flag==1){
      memset(&sim->ph1[0][0][0], 0, sim->n123pad*sizeof(float));
      memset(&sim->pv1[0][0][0], 0, sim->n123pad*sizeof(float));
    }else if(flag==2){
      memset(&sim->ph2[0][0][0], 0, sim->n123pad*sizeof(float));
      memset(&sim->pv2[0][0][0], 0, sim->n123pad*sizeof(float));
    }
  }
}
...
void fdtd_update_v(sim_t *sim, int flag, int it, int adj)
{
  ...
  float ***pv, ***ph, ***vz, ***vx, ***vy, ***memD1p, ***memD2p, ***memD3p;
  ...
  if(flag==0){
    pv = sim->aniso?sim->pv0:sim->p0;
    ph = sim->aniso?sim->ph0:sim->p0;
    vz = sim->vz0;
    vx = sim->vx0;
    vy = sim->vy0;
    memD1p = sim->memD1p0;
    ...
    pv = sim->aniso?sim->pv2:sim->p2;
    ph = sim->aniso?sim->ph2:sim->p2;
    vz = sim->vz2;
    vx = sim->vx2;
    vy = sim->vy2;
    memD1p = sim->memD1p2;
...
#pragma omp parallel for default(none)					\
  ...
  shared(i1min, i1max, i2min, i2max, i3min, i3max, _d1, _d2, _d3, dt,	\
	 flag, ph, pv, vz, vx, vy, memD1p, memD2p, memD3p, sim)
#endif  
  for(i3=i3min; i3<=i3max; i3++){
    for(i2=i2min; i2<=i2max; i2++){
      for(i1=i1min; i1<=i1max; i1++){
	if(sim->order==4){
	  D1p = 1.125*(pv[i3][i2][i1+1]-pv[i3][i2][i1])
	    -0.041666666666666664*(pv[i3][i2][i1+2]-pv[i3][i2][i1-1]);
	  D2p = 1.125*(ph[i3][i2+1][i1]-ph[i3][i2][i1])
	    -0.041666666666666664*(ph[i3][i2+2][i1]-ph[i3][i2-1][i1]);
	}else if(sim->order==8){
	  D1p = 1.196289062500000*(pv[i3][i2][i1+1]-pv[i3][i2][i1])
	    -0.079752604166667*(pv[i3][i2][i1+2]-pv[i3][i2][i1-1])
	    +0.009570312500000*(pv[i3][i2][i1+3]-pv[i3][i2][i1-2])
	    -0.000697544642857*(pv[i3][i2][i1+4]-pv[i3][i2][i1-3]);
	  D2p = 1.196289062500000*(ph[i3][i2+1][i1]-ph[i3][i2][i1])
	    -0.079752604166667*(ph[i3][i2+2][i1]-ph[i3][i2-1][i1])
	    +0.009570312500000*(ph[i3][i2+3][i1]-ph[i3][i2-2][i1])
	    -0.000697544642857*(ph[i3][i2+4][i1]-ph[i3][i2-3][i1]);
	}	
	D1p *= _d1;
	D2p *= _d2;
	...
	if(sim->aniso){
	  vz[i3][i2][i1] += dt*sim->buz[i3][i2][i1]*D1p;
	  vx[i3][i2][i1] += dt*sim->bux[i3][i2][i1]*D2p;
	}else{
	  vz[i3][i2][i1] -= dt*sim->buz[i3][i2][i1]*D1p;
	  vx[i3][i2][i1] -= dt*sim->bux[i3][i2][i1]*D2p;
	}
	if(flag==1){
	  sim->dvzdt[i3][i2][i1] = -sim->buz[i3][i2][i1]*D1p;
	  sim->dvxdt[i3][i2][i1] = -sim->bux[i3][i2][i1]*D2p;
	}
	if(flag==0){
	  sim->dvzdt0[i3][i2][i1] = -sim->buz[i3][i2][i1]*D1p;
	  sim->dvxdt0[i3][i2][i1] = -sim->bux[i3][i2][i1]*D2p;
	}
	
	if(sim->n3>1){
	  if(sim->order==4){
	    D3p = 1.125*(ph[i3+1][i2][i1]-ph[i3][i2][i1])
	      -0.041666666666666664*(ph[i3+2][i2][i1]-ph[i3-1][i2][i1]);
	  }else if(sim->order==8){
	    D3p = 1.196289062500000*(ph[i3+1][i2][i1]-ph[i3][i2][i1])
	      -0.079752604166667*(ph[i3+2][i2][i1]-ph[i3-1][i2][i1])
	      +0.009570312500000*(ph[i3+3][i2][i1]-ph[i3-2][i2][i1])
	      -0.000697544642857*(ph[i3+4][i2][i1]-ph[i3-3][i2][i1]);
	  }
	  D3p *= _d3;
	  if(i3<sim->nb){
	  ...
	  if(sim->aniso) vy[i3][i2][i1] += dt*sim->buy[i3][i2][i1]*D3p;
	  else	  vy[i3][i2][i1] -= dt*sim->buy[i3][i2][i1]*D3p;
	  if(flag==1) sim->dvydt[i3][i2][i1] = -sim->buy[i3][i2][i1]*D3p;
	  if(flag==0) sim->dvydt0[i3][i2][i1] = -sim->buy[i3][i2][i1]*D3p;
	}//end if n3>1
...
void fdtd_update_p(sim_t *sim, int flag, int it, int adj)
{
  ...
  int i1min, i1max, i2min, i2max, i3min, i3max;
  float ***p, ***pv, ***ph, ***vz, ***vx, ***vy, ***memD1vz, ***memD2vx, ***memD3vy;
  float dt = sim->sign_dt*sim->dt;
  float _d1 = 1./sim->d1;
  float _d2 = 1./sim->d2;
  float _d3 = 1./sim->d3;
...
#pragma omp parallel for default(none)					\
  ...
  shared(i1min, i1max, i2min, i2max, i3min, i3max, _d1, _d2, _d3, dt,	\
	 flag, p, pv, ph, vz, vx, vy, memD1vz, memD2vx, memD3vy, sim)
#endif  
  for(i3=i3min; i3<=i3max; i3++) {
    for(i2=i2min; i2<=i2max; i2++) {
      for(i1=i1min; i1<=i1max; i1++) {
...
#pragma omp parallel for default(none)			\
  schedule(static)					\
  private(i2, i3)					\
  shared(i2min, i2max, i3min, i3max, pv, ph, sim)
#endif  
      for(i3=i3min; i3<=i3max; i3++){
	for(i2=i2min; i2<=i2max; i2++){
	  pv[i3][i2][sim->nb] = 0;
	  pv[i3][i2][sim->nb-1] = -pv[i3][i2][sim->nb+1];

	  ph[i3][i2][sim->nb] = 0;
	  ph[i3][i2][sim->nb-1] = -ph[i3][i2][sim->nb+1];
	}
      }
    }else if(sim->order==8){
#ifdef _OPENMP
#pragma omp parallel for default(none)			\
  ...
  shared(i2min, i2max, i3min, i3max, pv, ph, sim)
	...
	for(i2=i2min; i2<=i2max; i2++){
	  pv[i3][i2][sim->nb] = 0;
	  pv[i3][i2][sim->nb-1] = -pv[i3][i2][sim->nb+1];
	  pv[i3][i2][sim->nb-2] = -pv[i3][i2][sim->nb+2];
	  pv[i3][i2][sim->nb-3] = -pv[i3][i2][sim->nb+3];

	  ph[i3][i2][sim->nb] = 0;
	  ph[i3][i2][sim->nb-1] = -ph[i3][i2][sim->nb+1];
	  ph[i3][i2][sim->nb-2] = -ph[i3][i2][sim->nb+2];
	  ph[i3][i2][sim->nb-3] = -ph[i3][i2][sim->nb+3];
	}
      }
